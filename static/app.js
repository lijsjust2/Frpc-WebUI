...
async function api(method, path, body = null, options = {}) {
    const { skipAuthRedirect = false, noRefresh = false } = options;
    const opts = {
        method,
...
async function init() {
    try {
        ...
        if (savedToken) {
            ...
            try {
                await api('GET', '/servers', null, { skipAuthRedirect: true, noRefresh: true });
                enterApp();
                return;
            } catch (e) {
        ...
        // Reflect Bark 2FA status on the login page
...
// Update the login page UI based on whether Bark 2FA is enabled
function updateLoginBarkUI(barkEnabled) {
    const codeGroup = document.getElementById('login-code-group');
    const subtitle = document.getElementById('login-subtitle');
    if (barkEnabled) {
        codeGroup.classList.remove('hidden');
        subtitle.textContent = '请输入账号、密码并完成 Bark 验证';
    } else {
        codeGroup.classList.add('hidden');
        subtitle.textContent = '请输入账号与密码';
        // Clear any in-progress code state
...
document.getElementById('setup-form').addEventListener('submit', async (e) => {
    ...
    if (username.length < 3) {
        errEl.textContent = '账号至少需要 3 个字符';
    ...
    try {
        const res = await api('POST', '/auth/setup', { username, password: pw });
        authToken = res.token;
        localStorage.setItem('authToken', authToken);
        toast('账号密码设置成功', 'success');
        enterApp();
    } catch (e) {
...
function setSendCodeCountdown(seconds) {
...
document.getElementById('btn-send-code').addEventListener('click', async () => {
    ...
    btn.textContent = '发送中...';
    try {
        const res = await api('POST', '/auth/send-code', { username, password }, { skipAuthRedirect: true });
        toast(res.message || '验证码已发送', 'success');
        setSendCodeCountdown(60);
    } catch (e) {
        const wait = e && e.data && typeof e.data.wait === 'number' ? e.data.wait : 0;
        if (wait > 0) {
            toast(e.message, 'error');
            setSendCodeCountdown(wait);
        } else {
            errEl.textContent = e.message;
            errEl.classList.remove('hidden');
            resetSendCodeButton();
...
document.getElementById('login-form').addEventListener('submit', async (e) => {
    ...
    const codeGroup = document.getElementById('login-code-group');
    const code = codeGroup.classList.contains('hidden') ? '' : document.getElementById('login-code').value.trim();
    const errEl = document.getElementById('login-error');
    ...
    if (!codeGroup.classList.contains('hidden') && !code) {
    ...
    errEl.classList.add('hidden');
    try {
        const res = await api('POST', '/auth/login', { username, password: pw, code }, { skipAuthRedirect: true });
        authToken = res.token;
        localStorage.setItem('authToken', authToken);
        resetSendCodeButton();
        document.getElementById('login-code').value = '';
...
document.getElementById('password-form').addEventListener('submit', async (e) => {
    ...
    if (!newUsername && !newPassword) {
        errorEl.textContent = '新账号和新密码不能同时为空';
    ...
    try {
        await api('POST', '/auth/change-credentials', { oldPassword, newUsername, newPassword });
        closeModal('modal-password');
        toast('账号密码已修改，请重新登录', 'success');
    } catch (e) {
...
async function loadBarkStatus() {
    try {
        const data = await api('GET', '/auth/bark');
        const infoEl = document.getElementById('bark-status-info');
        const enabledEl = document.getElementById('bark-enabled');
        if (data.configured) {
            document.getElementById('bark-server-url').value = data.serverUrl || '';
            document.getElementById('bark-device-token').value = '';
            document.getElementById('bark-device-token').placeholder = data.deviceTokenMasked || '留空表示不修改';
            enabledEl.checked = !!data.enabled;
            infoEl.textContent = data.enabled ? '当前已启用 Bark 二次验证。' : '当前已配置 Bark，但二次验证已关闭。';
        } else {
            document.getElementById('bark-server-url').value = '';
            document.getElementById('bark-device-token').value = '';
            document.getElementById('bark-device-token').placeholder = 'Bark App 中显示的 Device Token';
            enabledEl.checked = false;
            infoEl.textContent = '尚未配置 Bark 推送服务。';
        }
    } catch (e) {
        // ignore
    }
...
document.getElementById('btn-bark-settings').addEventListener('click', () => {
    document.getElementById('bark-error').classList.add('hidden');
    openModal('modal-bark');
    loadBarkStatus();
});

document.getElementById('bark-form').addEventListener('submit', async (e) => {
    e.preventDefault();
    ...
    const errorEl = document.getElementById('bark-error');

    if (!serverUrl) {
        errorEl.textContent = '请填写 Bark 服务地址';
        errorEl.classList.remove('hidden');
        return;
    }
    try {
        ...
        // 如果开关关闭，则同步启用状态
        if (!enabled) {
            await api('POST', '/auth/bark/enabled', { enabled: false });
        }
        closeModal('modal-bark');
        toast('Bark 配置已保存', 'success');
        // 刷新登录页状态
        try {
            const status = await api('GET', '/auth/status');
            updateLoginBarkUI(status.barkEnabled);
...
// Bark 启用开关即时切换
document.getElementById('bark-enabled').addEventListener('change', async function() {
    const errorEl = document.getElementById('bark-error');
...
document.getElementById('btn-bark-test').addEventListener('click', async () => {
    const errorEl = document.getElementById('bark-error');
    errorEl.classList.add('hidden');
    const btn = document.getElementById('btn-bark-test');
    const original = btn.textContent;
    const serverUrl = document.getElementById('bark-server-url').value.trim();
    const deviceToken = document.getElementById('bark-device-token').value.trim();

    if (!serverUrl) {
        errorEl.textContent = '请先填写 Bark 服务地址';
        errorEl.classList.remove('hidden');
        return;
    }
    ...
    try {
        ...
        await api('POST', '/auth/bark/test', Object.keys(body).length > 0 ? body : null);
        toast('测试消息已发送，请检查 Bark App', 'success');
    } catch (e) {
        errorEl.textContent = e.message;
        errorEl.classList.remove('hidden');
    } finally {
...
async function refreshLogs() {
    if (!selectedServerId) return;
    try {
        const data = await api('GET', `/servers/${selectedServerId}/logs`, null, { noRefresh: true });
...
// === Auto refresh ===
// 注意：后台轮询使用 X-No-Refresh: 1，不会续期会话。
// 这样可以保证“半小时无操作自动退出”的语义——只有用户主动操作（点击、提交等）才会刷新会话。
setInterval(async () => {
    if (authToken && document.getElementById('main-page').classList.contains('hidden') === false) {
        try {
            // 直接发起带 noRefresh 的请求获取最新状态，避免 loadServers() 内部再走默认（会续期）路径
            const data = await api('GET', '/servers', null, { noRefresh: true });
            servers = data;
            renderServerList();
            if (selectedServerId) {
                renderServerDetail();
